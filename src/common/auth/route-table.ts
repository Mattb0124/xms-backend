...
export function collectRouteTable(app: INestApplicationContext | ModuleRef): RouteEntry[] {
  ...
  for (const wrapper of discovery.getControllers()) {
    ...
    const controllerPath = normalise(reflector.get<string | string[]>(PATH_METADATA, metatype) ?? '');
    const controllerVersion = reflector.get<string | undefined>(VERSION_METADATA, metatype);
    const prototype = Object.getPrototypeOf(instance) as object;
    for (const name of scanner.getAllMethodNames(prototype)) {
      const handler = (prototype as Record<string, unknown>)[name] as ((...args: unknown[]) => unknown) | undefined;
      if (typeof handler !== 'function') continue;
      const methodCode = reflector.get<number | undefined>(METHOD_METADATA, handler);
      if (methodCode === undefined) continue;
      const routePath = normalise(reflector.get<string | string[]>(PATH_METADATA, handler) ?? '');
      const version = reflector.get<string | symbol | undefined>(VERSION_METADATA, handler) ?? controllerVersion;
      const prefix = version === undefined ? '/v1' : typeof version === 'symbol' ? '' : `/v${version}`;
...
export function undeclaredRoutes(table: RouteEntry[]): RouteEntry[] {
  return table.filter((entry) => entry.permission === null && entry.public === null);
}
