...
describe('review before send (DR-05, functional 5.8)', () => {
  ...
  it('holds the run, notifies the reviewers, and approve delivers exactly what was held', async () => {
    ...
    // The signer is handed the delivery lifetime, not the store's own
    // short default, and both renditions get the same one. Seven days is
    // the SigV4 maximum: a longer number is not a longer link, it is one
    // S3 refuses at redemption.
