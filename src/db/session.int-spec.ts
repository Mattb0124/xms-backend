...
beforeAll(async () => {
  ...
  await withSuperuser(async (client) => {
    ...
    await client.query(
      `insert into acct.account_settings (account_id) values ($1), ($2), ($3)`,
      [A, B, C],
    );
  });
});
...
describe('session binding', () => {
  it('returns no rows to a connection that never bound an account set', async () => {
    const bare = await pools()
      .get('app')
      .query('select count(*)::int as n from acct.account_settings');
  ...
  it('returns exactly the rows of the granted accounts', async () => {
    const seen = await withSession(
      pools(),
      'app',
      { binding: { kind: 'operator', accountIds: [A, C] } },
      (tx) =>
        tx.query<{ account_id: string }>(
          'select account_id from acct.account_settings order by account_id',
        ),
    );
    expect(seen.rows.map((row) => row.account_id).sort()).toEqual(
      [A, C].sort(),
    );
  });
    ...
    await withSession(
      ...
      async (tx) => {
        const seen = await tx.query(
          'select count(*)::int as n from acct.account_settings',
        );
  ...
  it('rejects a binding that is not made of uuids', async () => {
    await expect(
      withSession(
        pools(),
        'app',
        { binding: { kind: 'operator', accountIds: ["x'); drop table"] } },
        async () => 1,
      ),
  ...
  it('rolls back when the unit of work throws', async () => {
    await expect(
      withSession(
        ...
        async (tx) => {
          ...
          await tx.query(
            'update acct.account_settings set portal_enabled = true where account_id = $1',
            [A],
          );
    ...
    const row = await withSuperuser((client) =>
      client.query(
        'select portal_enabled from acct.account_settings where account_id = $1',
        [A],
      ),
    );
...
describe('audit guard', () => {
  ...
  it('never allows an audit event to be updated or deleted, even by the owner', async () => {
    await expect(
      withSuperuser((client) =>
        client.query(
          `update acct.audit_events set field = 'x' where account_id = $1`,
          [A],
        ),
      ),
    ...
    await expect(
      withSuperuser((client) =>
        client.query(`delete from acct.audit_events where account_id = $1`, [
          A,
        ]),
      ),
    ).rejects.toMatchObject({ code: '23001' });
  ...
  it('never allows a security event to be updated or deleted', async () => {
    ...
    await expect(
      withSuperuser((client) =>
        client.query(`delete from sys.security_events where actor_id = 'u1'`),
      ),
    ).rejects.toMatchObject({ code: '23001' });
  });
});
